...
private:
    ...
    //       their page/filename
    std::map<std::string, std::set<WebPage*>> index_;
    std::map<std::string, WebPage*> page_by_filename_;

    PageParser* getParser(std::string ext) const;
    WebPageSet getSetFromIndex(std::string term) const;
